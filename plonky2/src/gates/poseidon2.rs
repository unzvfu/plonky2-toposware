...
/// Evaluates a full Poseidon2 permutation with a custom number of state elements.
///
/// This also has some extra features to make it suitable for efficiently verifying Merkle proofs.
/// It has a flag which can be used to swap the first four inputs with the next four, for ordering
/// sibling digests.
#[derive(Debug, Default)]
pub struct Poseidon2Gate<F: RichField + Extendable<D>, const W: usize, const D: usize>(
    PhantomData<F>,
);

impl<F: RichField + Extendable<D>, const W: usize, const D: usize> Poseidon2Gate<F, W, D> {
    pub fn new() -> Self {
        Self(PhantomData)
    }
    ...
    const START_PARTIAL: usize = Self::START_FULL_0 + W * (poseidon2::HALF_N_FULL_ROUNDS - 1);
    ...
    fn end() -> usize {
        Self::START_FULL_1 + W * poseidon2::HALF_N_FULL_ROUNDS
    }
}

impl<F: RichField + Poseidon2<W> + Extendable<D>, const W: usize, const D: usize> Gate<F, D>
    for Poseidon2Gate<F, W, D>
{
    fn id(&self) -> String {
        format!("{self:?}<WIDTH={W}>")
    }
    ...
    fn eval_unfiltered(&self, vars: EvaluationVars<F, D>) -> Vec<F::Extension> {
        ...
        // Assert that `swap` is less than 4.
        let swap = vars.local_wires[Self::WIRE_SWAP];
        constraints.push(
            swap * (swap - F::Extension::ONE)
                * (swap - F::Extension::from_canonical_u64(2))
                * (swap - F::Extension::from_canonical_u64(3)),
        );
        ...
        <F as Poseidon2<W>>::external_matrix_field(&mut state);

        // First set of full rounds.
        for r in 0..poseidon2::HALF_N_FULL_ROUNDS {
            <F as Poseidon2<W>>::constant_layer_field(&mut state, round_ctr);
            if r != 0 {
                for i in 0..W {
            ...
            <F as Poseidon2<W>>::sbox_layer_field(&mut state);
            <F as Poseidon2<W>>::external_matrix_field(&mut state);
            round_ctr += 1;
        }
        ...
        for r in 0..(poseidon2::N_PARTIAL_ROUNDS) {
            state[0] += F::Extension::from_canonical_u64(
                poseidon2::ALL_ROUND_CONSTANTS[poseidon2::HALF_N_FULL_ROUNDS * W + r * W],
            );
            let sbox_in = vars.local_wires[Self::wire_partial_sbox(r)];
            constraints.push(state[0] - sbox_in);
            state[0] = <F as Poseidon2<W>>::sbox_monomial(sbox_in);
            <F as Poseidon2<W>>::internal_matrix_field(&mut state);
        }
    ...
    fn eval_unfiltered_base_one(
        ...
        // First external matrix
        <F as Poseidon2<W>>::external_matrix(&mut state);

        // First set of full rounds.
        for r in 0..poseidon2::HALF_N_FULL_ROUNDS {
            <F as Poseidon2<W>>::constant_layer(&mut state, round_ctr);
            if r != 0 {
                for i in 0..W {
                    let sbox_in = vars.local_wires[Self::wire_full_sbox_0(r, i)];
            ...
            <F as Poseidon2<W>>::sbox_layer(&mut state);
            <F as Poseidon2<W>>::external_matrix(&mut state);
            round_ctr += 1;
        }

        // Partial rounds.
        let mut constant_counter = poseidon2::HALF_N_FULL_ROUNDS * W;
        for r in 0..(poseidon2::N_PARTIAL_ROUNDS) {
            ...
            state[0] = <F as Poseidon2<W>>::sbox_monomial(sbox_in);
            <F as Poseidon2<W>>::internal_matrix(&mut state);
        }
        round_ctr += poseidon2::N_PARTIAL_ROUNDS;
    ...
    fn eval_unfiltered_circuit(
        ...
        // First external matrix
        <F as Poseidon2<W>>::external_matrix_circuit(builder, &mut state);

        // First set of full rounds.
        for r in 0..poseidon2::HALF_N_FULL_ROUNDS {
            <F as Poseidon2<W>>::constant_layer_circuit(builder, &mut state, round_ctr);
            if r != 0 {
                for i in 0..W {
                    let sbox_in = vars.local_wires[Self::wire_full_sbox_0(r, i)];
            ...
            <F as Poseidon2<W>>::sbox_layer_circuit(builder, &mut state);
            <F as Poseidon2<W>>::external_matrix_circuit(builder, &mut state);
            round_ctr += 1;
        }
        ...
        for r in 0..(poseidon2::N_PARTIAL_ROUNDS) {
            ...
            state[0] = <F as Poseidon2<W>>::sbox_monomial_circuit(builder, sbox_in);
            <F as Poseidon2<W>>::internal_matrix_circuit(builder, &mut state);
        }
        round_ctr += poseidon2::N_PARTIAL_ROUNDS;
    ...
    fn generators(&self, row: usize, _local_constants: &[F]) -> Vec<WitnessGeneratorRef<F>> {
        let gen = Poseidon2Generator::<F, W, D> {
            row,
    ...
    fn num_constraints(&self) -> usize {
        W * (poseidon2::N_FULL_ROUNDS_TOTAL - 1) + poseidon2::N_PARTIAL_ROUNDS + W + 1 + 4
    }
}

#[derive(Debug, Default)]
pub struct Poseidon2Generator<
    F: RichField + Extendable<D> + Poseidon2<W>,
    const W: usize,
    const D: usize,
> {
    row: usize,
...
impl<F: RichField + Extendable<D> + Poseidon2<W>, const W: usize, const D: usize> SimpleGenerator<F>
    for Poseidon2Generator<F, W, D>
{
    fn id(&self) -> String {
        format!("Poseidon2Generator<WIDTH={W}>")
    }

    fn dependencies(&self) -> Vec<Target> {
        (0..W)
            .map(|i| Poseidon2Gate::<F, W, D>::wire_input(i))
            .chain(Some(Poseidon2Gate::<F, W, D>::WIRE_SWAP))
    ...
    fn run_once(&self, witness: &PartitionWitness<F>, out_buffer: &mut GeneratedValues<F>) {
        ...
        let mut state = (0..W)
            .map(|i| witness.get_wire(local_wire(Poseidon2Gate::<F, W, D>::wire_input(i))))
            .collect::<Vec<_>>();

        let swap_value = witness.get_wire(local_wire(Poseidon2Gate::<F, W, D>::WIRE_SWAP));
        debug_assert!(swap_value == F::ZERO || swap_value == F::ONE);

        for i in 0..4 {
            let delta_i = swap_value * (state[i + 4] - state[i]);
            out_buffer.set_wire(local_wire(Poseidon2Gate::<F, W, D>::wire_delta(i)), delta_i);
        }
        ...
        for r in 0..poseidon2::HALF_N_FULL_ROUNDS {
            ...
            if r != 0 {
                for i in 0..W {
                    out_buffer.set_wire(
                        local_wire(Poseidon2Gate::<F, W, D>::wire_full_sbox_0(r, i)),
                        state[i],
                    );
                }
            }
        ...
        for r in 0..(poseidon2::N_PARTIAL_ROUNDS) {
            ...
            constant_counter += W;
            out_buffer.set_wire(
                local_wire(Poseidon2Gate::<F, W, D>::wire_partial_sbox(r)),
                state[0],
            );
            state[0] = <F as Poseidon2<W>>::sbox_monomial(state[0]);
            <F as Poseidon2<W>>::internal_matrix_field(&mut state);
        }
        round_ctr += poseidon2::N_PARTIAL_ROUNDS;
        ...
        for r in 0..poseidon2::HALF_N_FULL_ROUNDS {
            ...
            for i in 0..W {
                out_buffer.set_wire(
                    local_wire(Poseidon2Gate::<F, W, D>::wire_full_sbox_1(r, i)),
                    state[i],
                );
            }
            <F as Poseidon2<W>>::sbox_layer_field(&mut state);
        ...
        for i in 0..W {
            out_buffer.set_wire(
                local_wire(Poseidon2Gate::<F, W, D>::wire_output(i)),
                state[i],
            );
        }
    }
...
mod tests {
    ...
    fn wire_indices() {
        ...
        type Gate = Poseidon2Gate<F, 12, 4>;
    ...
    fn generated_output() {
        ...
        type Gate = Poseidon2Gate<F, 12, D>;
        let gate = Gate::new();
        ...
        let permutation_inputs = (0..12).map(F::from_canonical_usize).collect::<Vec<_>>();
        ...
        let expected_outputs: [F; 12] = F::poseidon2(permutation_inputs.try_into().unwrap());
        expected_outputs
            .iter()
    ...
    fn low_degree() {
        type F = GoldilocksField;
        let gate = Poseidon2Gate::<F, 12, 4>::new();
    ...
    fn eval_fns() -> Result<()> {
        ...
        type F = <C as GenericConfig<D>>::F;
        let gate = Poseidon2Gate::<F, 12, 2>::new();
